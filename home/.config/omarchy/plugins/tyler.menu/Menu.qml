...
Item {
  ...
  function testApps() {
    return JSON.stringify({
      usingFallback: !(root.shell && root.shell.appLibrary),
      appsCount: root.appLibrary ? root.appLibrary.sortedEntries("").length : -1
    })
  }
  ...
  // 4.0.3 sandboxes cloned menus: shell.appLibrary is null here, so the
  // Apps provider never fills. Use the host library when present, otherwise
  // read DesktopEntries locally and always launch a new window.
  readonly property var appLibrary: (root.shell && root.shell.appLibrary) ? root.shell.appLibrary : fallbackApps
  ...
  QtObject {
    ...
    function sortedEntries(query) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (!name) continue
        if (q && (name + " " + entrySubtext(entry)).toLowerCase().indexOf(q) < 0) continue
  ...
  Connections {
    ...
    function onValuesChanged() { fallbackApps.appsChanged() }
  }
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      opened = false
      filterText = ""
      // Cloned menu: always open a new window instead of focusing an existing one.
      Util.execDetached("gtk-launch " + Util.shellQuote(appId + ".desktop"))
    } else {
